...
\begin{abstract}
    Methods for curbing the spread of misinformation in the Philippines remain inadequate. The internet as a medium for fake news necessitates fast, automated, and accessible countermeasures. A precursor study from 2020 benchmarks deep learning models in building Filipino language fake news classifiers from a low-resource dataset. Despite promising results, the models from the aforementioned study cannot be feasibly deployed for a wide audience. In this work, we show that robust fake news classifiers for a morphologically rich language can be constructed from lightweight machine learning models. We show that these machine learning models can be deployed on a stringent infrastructure, with promising results. First, we construct a dataset of Filipino language news articles. Next, we extract Filipino linguistic features from the dataset. We train a Logistic Regression model, which achieves an accuracy of 95\% without hyperparameter tuning. Using this model, we build and deploy a system for classifying Filipino language fake news.
\end{abstract}
...
Misinformation ranks among the world's top global risks as fake news outlets see astounding engagement. In the Philippines, journalists and political analysts speculate that Former President Rodrigo Duterte's landslide victory in the 2016 presidential elections has been largely brought about by paid trolls disseminating fake news articles through social media outlets \cite{b1}. Former President Duterte earned tremendous publicity by popularizing a depiction of the Philippines as a \textit{narco-state}. Despite his insistence that this rhetoric is truth, the United Office on Drugs and Crime claims otherwise—the Philippines' drug use prevalence is lower than the global average \cite{b2}.

Previous studies explore deep learning models in building robust fake news classifiers for the morphologically rich Filipino language. Despite promising results, these models cannot be feasibly deployed for a wide audience due to high cost of deployment \cite{b3}. Therefore, this work investigates lightweight machine learning models in tackling the classification of Filipino language fake news. The authors deploy a cross-browser web extension that relies on a trained Logistic Regression model to classify Filipino language fake news.
