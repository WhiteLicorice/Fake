...
To understand the classification decisions of Logistic Regression deployed, coefficients of the predictors in the model were investigated. Table \ref{tab::lang_coef} shows the liguistic features and their coefficients.Since morphological and lexical features are not included in the deployment, only 19 predictors are considered. Negative coefficients mean that increase in the value of the predictor, while others are held constant, decreases the predicted probability, thus negative weights contribute to a prediction of class 0. It can be obeserved that average phrase count, average word length, and cvc density have the lowest coefficien. On the other hand, readability score, sentence count, stop word count, and OOV word count are the only linguistic features with postive coefficients. Furthermore, specific words also hold weights since bag-of-words and TF-IDFs are also used as predictors. Table \ref{tab::vec_coef} depicts the three predictors with highest and three with lowest coefficients from the vectorizers. 
...
Articles falsely classfied in section \ref{subsec::app-testing} are investigated. Their linguistic predictors are depicted in table \ref{tab::pred_false_class_linguistic}. Additionally, some of the words used in the two articles are also explored. As observed the false postive article have a relative higher readability score, sentece count, and OOV count. It also has a higher average phrase count but lower on other predictors with negative coefficients which is opposite for the false negative article. In addition to the linguistic predictors, word usage also has signifcant impact, infact some have higher magnitude in their coefficients as observed in table \ref{tab::vec_coef}. Shown in table \ref{tab::pred_false_class_vect} are examples of the two highest and three lowest vectorized words coefficients.

\begin{table}
    \begin{tabularx}{\textwidth}{|X|X|X|}
        \hline
        Predictor & False Postive (See Figure \ref{fig:false-positive-news-article}) & False Negative (See Figure \ref{fig:false-negative-news-article}) \\ 
        \hline
...
\begin{table}
    \begin{tabularx}{\textwidth}{|l|l|l|}
        ...
        \hline
        False Postive & vectorizers--bow--iba & -0.5394303591543086\\
        \hline
        False Postive & vectorizers--bow--supt & -0.432138317113013\\
        \hline
        False Postive & vectorizers--bow--ulat & 0.33552914902816716\\
        \hline
        False Postive & vectorizers--bow--office & 0.4609690671520561\\
        \hline
...
Deployment of the system with Logistic Regression as the classifier yields uniform results accross different browsers. Classifications depend on the linguistic features and specific word usage of articles since linguistic features, BoW, and TF-IDFs are used as predictors. Higher readability score and lower phrase count increases the probability of a class 1 preditction. Furthermore, false classification are also attributed to vectorized words since most a lot of them have relatively higher coefficient magnitude. Thus, word choice matters in the news articles. Vectorizers are fitted on the whole corpus, thus the writing style of the whole dataset would affect the values given by the vectorizers. This is in line with the previous hypothesis that writing style difference between the datasets affected the accuracy of the model.
