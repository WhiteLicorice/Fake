...
Figure. ROC Curves - Classical ML Classifiers (Joint Corpus, 30-Run CV). Lines show the mean ROC curve across unified CV folds; shaded bands show +/- 1 SD.

Figure. ROC Curves - All Models (Joint Corpus, 30-Run CV). Classical ML models are shown with solid lines and transformer models with dashed lines. Legend entries report mean ROC-AUC +/- SD from the saved fold-level predictions.

## Completion

- Requested models: MNB, RF, LR, SVC, DistilBERT, RoBERTa
- Saved fold metric rows: 180
- Full all-model target rows: 180
