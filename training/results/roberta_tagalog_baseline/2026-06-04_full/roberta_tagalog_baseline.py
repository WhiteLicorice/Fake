r"""Fine-tune RoBERTa-Tagalog for the FaKe reviewer baseline.
...
Run from the repository root with:
    ...
    training\venv\Scripts\python.exe training\roberta_tagalog_baseline.py
"""
...
N_REPEATS = 6
TEST_SIZE = 0.2
DEFAULT_OUTPUT_DIR = Path("results") / "roberta_tagalog_baseline"
PM = "\u00b1"
...
def configure_tee(run_dir: Path) -> Path:
    ...
    log_path = run_dir / "roberta_tagalog_baseline.log"
...
def training_args_for_fold(
    ...
    train_batch_size = train_batch_size or args.batch_size
    eval_batch_size = eval_batch_size or args.eval_batch_size
...
def note_batch_size_reduction(
    ...
    args.batch_size_reduced = True
    notes = getattr(args, "oom_notes", [])
    notes.append(
        {
    ...
    args.oom_notes = notes
    print(
        "CUDA OOM during fold; retrying with smaller batch size: "
        f"dataset={dataset_key} fold={fold_number} "
        f"failed_batch_size={failed_batch_size} retry_batch_size={next_batch_size}"
    )
...
def train_predict_one_fold(
    ...
    batch_sizes = fallback_batch_sizes(args.batch_size)
    for index, batch_size in enumerate(batch_sizes):
        ...
        try:
            ...
            if batch_size < args.batch_size:
                ...
                print(
                    "Using reduced batch size for remaining folds: "
            ...
            clear_torch_memory()
            if is_cuda_oom(exc) and index + 1 < len(batch_sizes):
                note_batch_size_reduction(
                    args=args,
                    dataset_key=dataset_key,
                    fold_number=fold_number,
                    failed_batch_size=batch_size,
                    next_batch_size=batch_sizes[index + 1],
                    exc=exc,
                )
                continue
            print(
                "Fold failed; logging error and continuing: "
                f"dataset={dataset_key} fold={fold_number} seed={fold_seed}"
            )
            traceback.print_exc()
            return None
        finally:
...
def create_report(
    ...
    report_path = run_dir / "roberta_tagalog_baseline_results.md"

    lines: list[str] = []
    lines.append("# RoBERTa-Tagalog Baseline Results")
    lines.append("")
    ...
    lines.append(
        "Hyperparameters: learning rate 2e-5, batch size "
        f"{getattr(args, 'initial_batch_size', args.batch_size)}"
        f"{' (reduced to ' + str(args.batch_size) + ' after CUDA OOM)' if getattr(args, 'batch_size_reduced', False) else ''}, epochs 3, "
    ...
    for dataset_key in ["cruz", "lupac", "combined"]:
        ...
        classifier = "RoBERTa-Tagalog" if dataset_key == "cruz" else ""
        lines.append(f"| {classifier} | {dataset_name} | {sd_acc:.5f} | {mean_acc:.3f} |")
    lines.append("")

    lines.append("## RoBERTa F1 And ROC-AUC")
    lines.append("")
    ...
    if not joint_rows.empty:
        ...
        lines.append(f"| RoBERTa-Tagalog | {mean_auc:.3f} {PM} {sd_auc:.3f} |")
    lines.append("")
    ...
    lines.append(
        "Joint corpus, fold "
        f"{representative_metadata.get('fold_number', 'n/a')}, selected as the "
        "fold closest to the joint-corpus mean accuracy. Labels: Fake = 0, Real = 1."
    )
    lines.append("")
    lines.append("| Classifier | Actual Class | Predicted Fake | Predicted Real |")
    lines.append("|---|---|---:|---:|")
    lines.append(
        "| RoBERTa-Tagalog | Actual Fake | "
    ...
    lines.append("|---|---:|---:|")
    lines.append("| Logistic Regression | 1,451,808 | 66.04 MB |")
    lines.append(
        f"| RoBERTa-Tagalog | {parameter_count:,} | {model_size_mb:.2f} MB |"
    )
    lines.append("")
    lines.append(
    ...
    if getattr(args, "oom_notes", []):
        lines.append("")
        lines.append("CUDA OOM retry details are saved in `batch_size_notes.json`.")
    lines.append("")
...
def print_environment(args: argparse.Namespace, run_dir: Path) -> None:
    print("RoBERTa-Tagalog baseline run")
...
def evaluate_dataset(
    ...
    for fold_number, (train_index, val_index) in enumerate(split_iter, start=1):
        ...
        fold_result = train_predict_one_fold(
            dataset_key=dataset_key,
            fold_number=fold_number,
            fold_seed=fold_seed,
        ...
        if representative_parameter_count is None:
            representative_parameter_count = int(fold_result["parameter_count"])

        labels = fold_result["labels"]
        logits = fold_result["logits"]
        elapsed = float(fold_result["elapsed_seconds"])
        ...
        print(
            ...
            f"batch_size={fold_result['batch_size']}"
        )
        print(
            "confusion_matrix="
...
def select_representative_joint_fold(metrics_path: Path) -> dict[str, object]:
    metrics = pd.read_csv(metrics_path)
    joint = metrics[metrics["dataset_key"] == "combined"].copy()
    if joint.empty:
        raise RuntimeError("Cannot select representative fold without combined metrics.")
...
def train_representative_joint_model(
    metrics_path: Path,
    tokenizer,
    run_dir: Path,
    args: argparse.Namespace,
) -> tuple[list[list[int]], Path, dict[str, object]]:
    representative = select_representative_joint_fold(metrics_path)
    fold_number = int(representative["fold_number"])
    fold_seed = int(representative["seed"])
    _, joint_frame = load_dataset_frame("combined")
    X_train_fold, X_val_fold = combined_fold_frames(joint_frame, fold_number, args)

    model_dir = run_dir / "representative_joint_model"
    fold_dir = run_dir / "representative_joint_model_training"
    print(
        "\nTraining representative joint model from closest-to-mean fold: "
        f"fold={fold_number} seed={fold_seed} "
        f"fold_accuracy={float(representative['accuracy']):.6f} "
        f"joint_mean_accuracy={float(representative['joint_mean_accuracy']):.6f}"
    )
    fold_result = train_predict_one_fold(
        dataset_key="combined_representative",
        fold_number=fold_number,
        fold_seed=fold_seed,
        X_train_fold=X_train_fold,
        X_val_fold=X_val_fold,
    ...
    if fold_result is None:
        raise RuntimeError("Representative joint model training failed.")
    ...
    representative.update(
        {
            ...
            "representative_rerun_accuracy": metrics["accuracy"],
            "representative_rerun_f1_macro": metrics["f1_macro"],
            "representative_rerun_roc_auc": metrics["roc_auc"],
            "representative_rerun_batch_size": fold_result["batch_size"],
        }
    )
    print(
        "Representative rerun metrics: "
        f"accuracy={metrics['accuracy']:.4f} "
        f"f1_macro={metrics['f1_macro']:.4f} "
        f"roc_auc={metrics['roc_auc']:.4f}"
    )
    print(f"Saved representative joint model to {model_dir}")
    return cm, model_dir, representative


def main() -> None:
    args = parse_args()
    args.initial_batch_size = args.batch_size
    args.batch_size_reduced = False
    args.oom_notes = []
    ...
    for dataset_key in dataset_keys:
        ...
        _, _, fold_parameter_count = evaluate_dataset(
            dataset_key,
            dataset_name,
            frame,
            tokenizer,
    ...
    if "combined" not in dataset_keys:
        raise RuntimeError(
            "No representative joint fold can be produced. Include the combined dataset."
        )
    ...
    representative_cm, representative_model_dir, representative_metadata = (
        train_representative_joint_model(metrics_path, tokenizer, run_dir, args)
    )
    ...
    write_json(run_dir / "representative_joint_model_metadata.json", representative_metadata)
    if args.oom_notes:
        write_json(run_dir / "batch_size_notes.json", args.oom_notes)
